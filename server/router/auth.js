...
router.get("/my-profile", async (req, res) => {
  ...
  try {
    if (token) {
      ...
      const TeacherData = await TeachersModel.findOne({
        "tokens.token": token,
      });
      ...
      if (UserData) {
        ...
        return res.status(200).send(TeacherData);
      } else if (AdminData) {
...
router.post("/add-task", async (req, res) => {
  ...
  try {
    const taskAssignedExist = await TasksModal.findOne({
      selected_student_id: selected_student_id,
    });
    const taskDescriptionExist = await TasksModal.findOne({
      description: description,
    });
    if (taskAssignedExist && taskDescriptionExist) {
      return req
        .status(402)
        .send(console.log("Please enter the valid task name or description"));
    } else {
      const taskData = await TasksModal({
        name,
        description,
        selected_student_id,
        login_teacher_id,
...
router.post("/add-student", async (req, res) => {
  ...
  try {
    ...
    if (existUser || AdminExist || TeacherExist) {
      return res
        .status(201)
        .send(console.log("Please enter valid Email credentials."));
    } else if (Password != cPassword) {
      return res
        .status(201)
        .send(console.log("Please enter valid Password credentials."));
    } else {
      const studentData = await StudentsModel({
        Name,
        Email,
        Phone,
        Age,
...
router.post("/add-teacher", async (req, res) => {
  ...
  if (
    ...
    return res.status(204).send("Please enter all the required fields.");
  }
  try {
    const teacherexist = await TeachersModel.findOne({ Email: Email });
    ...
    if (teacherexist || adminexist || studentexist) {
      ...
      const teacherData = await TeachersModel({
        Name,
        Email,
        Phone,
        Age,
...
router.post("/signin", async (req, res) => {
  try {
    ...
    if (teacherLogin) {
      const teacherPasswordMatch = await bcrypt.compare(
        Password,
...
router.post("/add-book", async (req, res) => {
  try {
    ...
    if (!BookName || !Class || !file) {
      ...
      const BookData = await LibraryBooksModel({
        BookName,
        Class,
        Semester,
        ThumbnailPath,
