...
const TeacherProfile = () => {
  ...
  const [updatedOnlineTeachingExperience, setUpdatedOnlineTeachingExperience] =
    useState("");
  const [updatedTeachingExperience, setUpdatedTeachingExperience] =
    useState("");
    ...
    try {
      ...
      Axios.get("http://localhost:3001/my-profile", {
        ...
        if (response.status === 200) {
          setUserData({
            ...response.data,
            Image: response.data.Image, // Ensure this is the correct path to the updated image
          });
  ...
  async function UpdateData(e) {
    if (
      ...
      !updatedOnlineTeachingExperience ||
      !Description
    ) {
      ...
      try {
        const response = await Axios.put(
          "http://localhost:3001/update-teacher",
          UpdatedData
        );
        if (response.status === 200) {
    ...
    if (Image && Image.type.startsWith("image/")) {
      try {
        Axios.post("http://localhost:3001/update-img", imageData)
          .then((response) => {
            if (response.status === 200) {
              setUserData(response.data); // Update with the new data
              navigate("/teacherprofile");
            } else {
              alert("Your Data Didn't Update!");
            }
                  ...
                  <div className="mx-2">
                    ...
                    <div className="form-group mt-3">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setUpdatedOnlineTeachingExperience(event.target.value)
                        }
                      />
                    </div>
                    <div className="form-group mt-3">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setUpdatedTeachingExperience(event.target.value)
                        }
                      />
                    </div>
                    <div className="form-group mt-3">
